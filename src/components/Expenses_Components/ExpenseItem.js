...
//The ExpenseItem function/component accepts the props(properties) - title,date & amount as an argument from the Expenses.js file/component
//Since it is like a child to it i.e the data from this component is used inside the Expenses component.
//I can now do whatever i want to do with the props here before sending them to Expenses component.
const ExpenseItem = (props) => {
  ...
  /* const [title, setTitle]/**The first element 'title' in the array is the old value for title while the second 'setTitle' is the updating function that we will call to set the new value as below */
  // = useState(props.title);//the imported react default function(hook) - useState() function
  ...
  return (
    ...
    <li>
      <Card className="expense-item">
        ...
        {/**Events, this has been uncommented out cos it was just a demo*/}
        {/**<button onClick={clickHandler /**we don't add the () after the clickhandler function cos we don't want the action to take place unless the button is clicked but if we had added it, the function would have been executed immediately the return block is been executed */}
        {/*>Change Title</button>*/}
      </Card>
    </li>
  );
